...
library(lmtest) #Para evaluación de supuestos
...
En esta sección, se lleva a cabo un análisis exploratorio de los datos con el fin de investigar su variación y balance, establecer si existe relación entre variables cuantitativas y factores, identificar si existen errores, datos faltantes o valores atípicos, así como aplicar estadística descriptiva para su resumen.
...
ggplot(Datos_Metaloproteasas_Norm, aes(x = Keratinolytic_activity, y = Positivity, fill=Keratinolytic_activity)) + geom_boxplot() + labs(title = "Positividad vs Actividad queratinolítica", x= "Actividad queratinolítica", y= "Positividad")

```

En este gráfico es obsrvable también la presencia de **valores atípicos** para metalo proteasas queratinolíticas, tres por sobre y dos por debajo del resto de los valores. 
...
names(pI_stat)[names(pI_stat) == 'Keratinolytic_activity'] <- 'Actividad Queratinolítica'
...
names(GRAVY_stat)[names(GRAVY_stat) == 'Keratinolytic_activity'] <- 'Actividad Queratinolítica'
...
names(Neg_stat)[names(Neg_stat) == 'Keratinolytic_activity'] <- 'Actividad Queratinolítica'
...
names(Pos_stat)[names(Pos_stat) == 'Keratinolytic_activity'] <- 'Actividad Queratinolítica'
...
names(AI_stat)[names(AI_stat) == 'Keratinolytic_activity'] <- 'Actividad Queratinolítica'
...
En esta sección, a partir de los resultados obtenidos del análisis exploratorio previo, se selecciona el indice de inestabilidad como variable de estudio, se propone una hipótesis y realiza un análisis estadístico de los datos para ponerla a prueba, evaluando los supuestos correspondientes a la prueba empleada. Finalmente, los resultados obtenidos son presentados e interpretados para realizar una conclusión.

*** 
### Propuesta de hipótesis

A partir del análisis exploratorio de datos, se proponen las siguientes hipótesis estadísticas:

* *Hipótesis nula (H0): El indice de inestabilidad es igual entre metaloproteasas queratinolíticas y no-queratinolíticas.*

* *Hipótesis alternativa (HA): El indice de inestabilidad es distinto entre metaloproteasas queratinolíticas y no-queratinolíticas.*

***
### Evaluación de supuestos
Con el fin de seleccionar la prueba estadística apropiada, se realiza la evaluación de los siguientes supuestos:  

#### **Independencia**

Se evalúa empleando la prueba de Durbin-Watson, que considera las siguientes hipótesis:
...
dwtest(Instability_index ~ Keratinolytic_activity, data = Datos_Metaloproteasas_Norm, alternative = c("two.sided"), iterations = 15)

```
Dados estos resultados, considerando que el valor de p es inferior a 0,05, podemos rechazar la hipótesis nula y concluir que los residuos están autocorrelacionados.  



#### **Homocedasticidad**

Se evalúa empleando la prueba de Levene, que considera las siguientes hipótesis:

* H0: La varianza entre los grupos es igual. 

* HA: La varianza entre los grupos no es igual.

```{r}

leveneTest(Instability_index ~ Keratinolytic_activity, data =  Datos_Metaloproteasas_Norm, center = "median")

```
Dados estos resultados, considerando que el valor de p es superior a 0,05, no es posible rechazar la hipótesis nula y por lo tanto se concluye que la varianza entre los grupos es igual.  


#### **Normalidad**

Se evalúa empleando la prueba de Shapiro-Wilk, que considera las siguientes hipótesis:
...
lm.aov <- lm(Instability_index ~ Keratinolytic_activity, data =  Datos_Metaloproteasas_Norm)
aov_residuals <- residuals(object = lm.aov)
shapiro.test(x = aov_residuals)

```
Dados estos resultados, considerando que el valor de p es superior a 0,05, no es posible rechazar la hipótesis nula y por lo tanto podemos suponer que los datos de la muestra provienen de una población que se distribuye normalmente.

***
### Prueba de hipótesis

Considerando que, de acuerdo con los resultados de evaluación de supuestos, no existe independencia entre las muestras, hay igualdad de varianza y los datos provienen de una población que se distribuye normalmente, se determina realizar una prueba T pareada.

```{r}

t.test(Instability_index ~ Keratinolytic_activity, data = Datos_Metaloproteasas_Norm, alternative = c("two.sided"), paired = T, var.equal = T)

```
***
### Interpretación de resultados y conclusiones

Dados estos resultados, considerando que el valor de p es superior a 0,05, no es posible rechazar la hipótesis nula y por lo tanto se concluye que el indice de inestabilidad es igual entre metaloproteasas queratinolíticas y no-queratinolíticas.
